...
/**
 * 统计模块 - 每日花费趋势（2.12.0，重点模块）
 * 折线图展示当前月每日支出；底部摘要：本月总支出 / 日均支出 / 最高消费日。
 * 数据来自 statistics-modules 纯函数；图表生命周期由 useModuleChart 统一管理（持久 DOM）。
 */
defineOptions({ name: 'DailyExpenseTrendModule' });
import { computed, ref } from 'vue';
import type { EChartsCoreOption } from 'echarts/core';
import { DVCard } from '@/components/design';
import { useAppStore } from '@/core/store/app';
import type { Bill } from '@/core/models/types';
import { computeDailyExpenseTrend } from './statistics-modules';
...
const data = computed(() =>
  computeDailyExpenseTrend(props.bills, props.activeYm, props.today),
);

const chartRef = ref<HTMLElement | null>(null);
const theme = useModuleChart(
  chartRef,
  (): EChartsCoreOption => {
    const t = theme.theme();
      ...
      series: [
        {
          name: '每日支出',
          type: 'line',
          data: days.map((p) => p.amount),
          symbol: 'circle',
          symbolSize: 4,
          lineStyle: { color: t.expense, width: 2 },
...
<template>
  <DVCard class="stats-module" outlined>
    <div class="stats-module__head">
      <h3 class="stats-module__title">每日花费趋势</h3>
      <span class="stats-module__range">{{ monthLabel }}</span>
    </div>
    <div class="stats-module__stage">
      <div
        ref="chartRef"
        class="stats-module__chart"
        :class="{ 'is-hidden': !data.hasData }"
      />
    ...
    <div v-if="data.hasData" class="stats-module__foot">
      ...
      <span class="stats-module__metric">
        <b>日均支出</b><em>¥ {{ data.dailyAverage.toFixed(2) }}</em>
      </span>
      <span class="stats-module__metric">
        <b>最高消费日</b><em>{{ data.highestDay?.label ?? '—' }} ¥ {{ data.highestDay?.amount.toFixed(2) ?? '0.00' }}</em>
      </span>
    </div>
  </DVCard>
</template>
